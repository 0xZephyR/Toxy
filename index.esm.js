...
var Administration = function () {
  ...
  Administration.prototype.fresh = function () {
    ...
    (_a = this.freshMethods) === null || _a === void 0 ? void 0 : _a.forEach(function (v) {
...
var createStore = Object.assign(create, factory);
...
var batchQueue = {
  ...
  run: function run(delay) {
    ...
    this.id = setTimeout(function () {
      _this.reactionQueue.forEach(function (r) {
        return r.runReaction();
      });

      _this.reactionQueue.clear();

      _this.storeQueue.forEach(function (s) {
        return getAdm(s).fresh();
      });
...
var observableHandler = {
  get: function get(target, prop) {
    var model = this.this_; // 若不处于autorun上下文中，只返回值
  ...
  set: function set(target, prop, value, receiver) {
    ...
    var model = this.this_; // 检查数据是否有更新，若无更新，则不通知observer
...
function toDeepProxy(object_, handler_) {
  if (!isPureObject(object_)) {
    addSubProxy(object_, handler_);
  }

  return new Proxy(object_, handler_); //这是一个递归函数，目的是遍历object的所有属性，如果不是pure object,那么就继续遍历object的属性的属性，如果是pure object那么就加上proxy
...
var Model = function () {
  function Model(target) {
    ...
    var handler = __assign(__assign({}, observableHandler), {
      this_: this
    });
  ...
  Model.prototype.updateBatch = function (target, prop, delay) {
    ...
    if (set) {
      set.forEach(function (v) {
        return batchQueue.addReaction(v);
      });
    }
  ...
  Model.prototype.runObservers = function (target, prop) {
    ...
    (_b = (_a = this._observables.get(target)) === null || _a === void 0 ? void 0 : _a.get(prop)) === null || _b === void 0 ? void 0 : _b.forEach(function (value) {
      return value.runReaction();
    });
  };
...
function useNormalDerivation(root_) {
  var root = root_; // if (!root.isMounted()) {
...
export { autorun, createModel, createStore as default, transaction, useMainDerivation, useNormalDerivation };
